---
title: "Cell Line Drug Response Analysis"
...
CTRP_breast <- CTRP %>% 
  ...
  select(CCL_Name, Tissue, Cellosaurus_Id, cpd_name, Avg_AUC, apparent_ec50_umol) %>% 
...
CTRP_breast <- CTRP_breast %>% 
  mutate(Subtype= if_else(is.na(Subtype), missing_subtype[CCL_Name], Subtype)) %>%
  select(cpd_name, CCL_Name, Subtype, Avg_AUC, apparent_ec50_umol) %>% 
  unique()
...
CTRP_breast %>% 
  filter(cpd_name == "leptomycin B") %>% 
  arrange(Avg_AUC) %>% 
  ggplot(aes(x = reorder(CCL_Name, Avg_AUC), y = Avg_AUC)) + 
  geom_col() + 
...
CTRP_breast %>% 
  filter(cpd_name == "leptomycin B") %>% 
  mutate(log_ec50 = log(apparent_ec50_umol*1000)) %>% 
  filter(log_ec50 > 0) %>% 
  ggplot(aes(x = reorder(CCL_Name, apparent_ec50_umol), y = log_ec50, fill = TNBC)) + 
  geom_col(color = "dark grey") + 
  theme(axis.text.x = element_blank(),
        legend.position = c(0.12, 0.85)) +
  labs(x = "Cell Lines by Sensitivity",
       y = "log(EC50) in nM", 
       title = "CTRP Cell Line sensitivity to XPO1 Inhibitor Leptomycin B",
       fill = "TNBC Status") + 
  scale_fill_manual(labels = c("RPBC Cell Line", "TNBC Cell Line"), values = c("dark grey","light blue"))
